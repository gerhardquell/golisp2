...
🇩🇪 [Deutsch](README.md) · 🇬🇧 English · 🇨🇳 [中文](README_CN.md)
...
- **SWANK TCP Server**: real SWANK protocol (`:emacs-rex`) for IDE integration
...
- **File uploads**: `http-upload` registers a POST upload handler (`(lambda (name content) ...)`)
...
- **`:host` / `:tls`**: bind to LAN interfaces, serve HTTPS
...
(length-prefixed `:emacs-rex` RPC — no custom protocol):

```bash
# Terminal 1: Start the server
./build/golisp2 --swank 127.0.0.1:4321
# => SWANK server listening on 127.0.0.1:4321
...
> *"Code = Data + AI = a self-extending system"*
> — Gerhard & Claude
...
- 🇩🇪 [`README.md`](README.md) — Deutsche Projektbeschreibung
