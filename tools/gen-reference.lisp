...
;; Die Beschreibungstexte leben hier in *ref-docs* (nicht in der
;; Markdown-Datei!) -- so ueberleben sie jeden Regenerationslauf.
;; Neue Symbole ohne Eintrag erscheinen mit leerer Beschreibungsspalte.
;; Aufruf: ./build/golisp2 tools/gen-reference.lisp
;;**********************************************************************

(define *ref-out* "docs/referenz-generiert.md")

;; Symbol -> Kurzbeschreibung (eine Zeile). Pflege hier, nicht im Markdown.
(define *ref-docs*
...
;; ref-lookup: Beschreibung zum Symbolnamen (String). nil kann kein
;; Alisten-Key sein (Reader liest nil als leere Liste, intern "nil"
;; matcht nicht) -- deshalb Sonderfall.
(defun ref-lookup (name)
  (if (equal? name "nil")
      "NIL-Konstante (Singleton, MakeNil)"
      (let ((hit (assoc (intern name) *ref-docs*)))
        (if hit (cdr hit) ""))))

(defun write-reference ()
  (let* ((names (env-symbols))
         (rows (mapcar (lambda (n)
                         (let ((doc (ref-lookup n)))
                           (if (equal? doc "")
                               (format nil "| `~a` | |~%" n)
                               (format nil "| `~a` | ~a |~%" n doc))))
                       names))
         (header (format nil "# GoLisp2 — Generierte Funktionsreferenz~%~%> Automatisch erzeugt aus `(env-symbols)`. Vollständigkeit ist strukturell garantiert. Die Beschreibungstexte werden in `tools/gen-reference.lisp` gepflegt (*ref-docs*) — NICHT in dieser Datei, sie wird bei jedem Lauf komplett überschrieben. Neue Symbole ohne *ref-docs*-Eintrag erscheinen mit leerer Beschreibungsspalte.~%> Neu generieren: `./build/golisp2 tools/gen-reference.lisp`~%~%| Symbol | Beschreibung |~%|---|---|~%"))
         (body (apply string-append rows)))
    (file-write *ref-out* (string-append header body))
    (let ((filled (length (filter (lambda (n) (not (equal? (ref-lookup n) ""))) names))))
      (format t "~a Symbole nach ~a geschrieben (~a mit Beschreibung, ~a leer)~%"
              (length names) *ref-out* filled (- (length names) filled)))))
